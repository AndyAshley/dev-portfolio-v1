    ...
    return (
      <div className={classNames} aria-hidden={honeypot}>
        <label htmlFor={name} className="font-bold mb-1">
          {label}
          {required && <span className="text-cyber-green-400 ml-1">*</span>}
        </label>
        <input
          ref={ref}
          type={type}
          name={name}
          className="border border-gray-700 hover:border-cyber-green-400 focus-visible:border-cyber-green-400 outline-none transition-all duration-500 rounded shadow px-2 h-8 text-sm"
          required={required}
