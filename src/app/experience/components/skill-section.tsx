  ...
  return (
    <FadeInContent from="bottom">
      <div className="flex flex-row items-center justify-center">
        <Computer className="mr-2 text-cyber-green-400" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
        {SKILLS.map(({ category, items }, index) => (
          <ContentContainer key={index}>
            <h3 className="font-bold mb-2 pb-2 border-b border-gray-700">
              {category}
            </h3>
            <ul className="gap-2 list-none m-0">
              {items.map((skill, index) => (
                <li key={index} className="flex items-center">
                  <ChevronRight size={18} className="text-cyber-green-400" />
                  {skill}
                </li>
              ))}
            </ul>
